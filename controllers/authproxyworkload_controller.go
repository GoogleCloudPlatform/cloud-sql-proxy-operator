...
const shortRequeueAfter = 1 * time.Second
...
func (r *AuthProxyWorkloadReconciler) doAddUpdate(ctx context.Context, req ctrl.Request, resource *cloudsqlapi.AuthProxyWorkload, l logr.Logger) (ctrl.Result, error) {
	...
	if !controllerutil.ContainsFinalizer(resource, finalizerName) {
		...
		return ctrl.Result{Requeue: true, RequeueAfter: shortRequeueAfter}, err
	}
	...
	if len(workloads) == 0 {
		...
		err = r.patchAuthProxyWorkloadStatus(ctx, orig, resource)
		l.Info("Reconcile found no workloads to update", "ns", resource.GetNamespace(), "name", resource.GetName())
		return ctrl.Result{}, err
	}

	// Begin updating the workloads
	existingUpToDateStatus := meta.FindStatusCondition(resource.Status.Conditions, cloudsqlapi.ConditionUpToDate)
	if existingUpToDateStatus == nil ||
		(existingUpToDateStatus.Status == metav1.ConditionFalse && existingUpToDateStatus.Reason != "StartingWorkloadReconcile") {
		...
		l.Info("Reconcile launched workload updates", "ns", resource.GetNamespace(), "name", resource.GetName())
		return ctrl.Result{Requeue: true, RequeueAfter: shortRequeueAfter}, err
	}
	...
	l.Info("Reconcile completed workload updates", "ns", resource.GetNamespace(), "name", resource.GetName())
	return result, err
}

func (r *AuthProxyWorkloadReconciler) patchAuthProxyWorkloadStatus(ctx context.Context, orig *cloudsqlapi.AuthProxyWorkload, resource *cloudsqlapi.AuthProxyWorkload) error {
...
func makeWorkloadUpdate(workload internal.Workload, newAnnotations map[string]string) controllerutil.MutateFn {
	return func() error {
		workload.Object().SetAnnotations(newAnnotations)
		return nil
	}
}
...
func (r *AuthProxyWorkloadReconciler) saveWorkloads(ctx context.Context, wls []internal.Workload, l logr.Logger) {
	for _, wl := range wls {
		_, err := controllerutil.CreateOrPatch(ctx, r.Client, wl.Object(), makeWorkloadUpdate(wl, wl.Object().GetAnnotations()))
		if err != nil {
