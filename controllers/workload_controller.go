...
// NeedsUpdateAnnotation is added to all workloads when an AuthProxyWorkload changes and needs an update
const NeedsUpdateAnnotation = "cloudsql.cloud.google.com/authproxyworkload-needs-update"

// WasUpdatedAnnotation is added to all workloads when after an AuthProxyWorkload changes the update is applied
...
func (a *WorkloadAdmissionWebhook) Handle(ctx context.Context, req admission.Request) admission.Response {
	...
	if updated {
		l.Info("/mutate-workload request completed. The workload was updated.", "kind", req.Kind.Kind, "ns", req.Namespace, "name", req.Name)
	}
